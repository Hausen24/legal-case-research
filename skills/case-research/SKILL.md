...
### 2.4 相关度筛查（按模式分支）
**实案模式**：对分析池逐条按 `methodology/relevance-screening.md` 的**三要素比对量表**（基本事实/争议焦点/法律适用，《类案检索规定》第六条）与基准案情比对定档：高（≈90%+ 标识）/中（≈80%+ 标识）保留，低剔除（留痕）；每案记 `相关度` + `相关度依据`。仅当高+中为 0 时按兜底规则经确认纳入低相关并特别提示。
