...
> 实案四顺位（第四条）：①最高法**指导性案例** → ②最高法**典型案例及裁判生效案件** → ③**本省高院参考性案例及裁判生效案件** → ④**上一级法院及本院裁判生效案件**。默认全顺位检索，标注"前一顺位命中可不再检索"规则与首个命中顺位。

**开关二：案件形态**

| | **散案**（默认） | **集团/群体性案件** |
|---|---|---|
| 特征 | 个案彼此独立 | 一个共同事件派生大量平行、近似判决（众多当事人诉同一被告主体同一事件，如证券虚假陈述、产品责任集体维权、环境侵权） |
| 去重 | 按 `Gid` 去重 | **按"被告主体＋共同事件"分组**，组内识别核心判决（示范判决/代表人诉讼/说理最完整者），平行判决仅留痕（详见 `methodology/core-judgment-and-comparison.md`） |
...
> 两开关正交组合：学理×散案（经典类案研究）、学理×集团案（如京沪证券裁判规则研究）、
> 实案×散案（普通个案的类案检索报告）、**实案×集团案**（如本仓库实战的甲股份证券虚假陈述案）。
