...
### 2.4 检索覆盖率自检
运行 `python3 scripts/check_coverage.py <research_dir>`：生成 `07_coverage.json`（轮替词命中矩阵含"独有命中"、案件等级/法院层级分布、track 分布、去重审计、名录缺口）。独有命中为 0 的轮替词提示边际贡献存疑。本结果是检查点 2 的量化底稿与报告附录"检索过程说明"的数据源（法发〔2020〕24号第八条要素）。

### 🛑 检查点 2（必须停下）
报告：轮替词与各轮命中数、过滤后数、**分组数（列出覆盖的事件/主体清单）**、核心判决数、平行留痕数、典型案例数、**覆盖缺口（附覆盖率自检摘要）**。请用户校核分组与核心判决判定。获确认后进入第三步。
...
### 3.2 联网交叉核对典型案例
联网核对目标法院历年典型/十大/参考案例中的同类案，命中者标名录；名录中有而样本缺的列缺口补检。**核对结果落盘 `<research_dir>/名录.json`**（格式 `[{"名称":…,"案号":…,"来源":…}]`），随后重跑 `check_coverage.py` 即可得到机器化的名录命中/缺口表。**红线**：名录可联网，裁判事实/观点/案号仍以 MCP 为准，不得据网络编造。

**编码完成后立即校验**：运行 `python3 scripts/validate_pipeline.py <research_dir>`，校验 03–06 数据契约（字段类型/枚举/问题名拼写一致性）。有 error 必须修复后再进入 §3.3。
...
- **分歧地图**：同一问题对立倾向并存的问题清单 + 各倾向代表案对 + 地域观察（法发〔2020〕24号第十一条情形的显性化）。是 Excel「裁判分歧清单」sheet 与报告分歧表述的数据源；地域差异措辞照抄 `措辞` 字段口径。
...
├── 06_analytics.json                                 样本量自适应分析 + 分歧地图（不在成品中提及其文件名）
├── 07_coverage.json / 名录.json                       覆盖率自检 / 典型案例名录（核对底稿）
└── output/
