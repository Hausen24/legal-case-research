...
直接引文可在原文找到；**不能证明全部转述内容无误**，「」标记的术语/归纳语亦不在
核验范围（哨兵仅对超长/裁判语体的「」片段发警示）。转述质量由编码抽检
...
# 「」豁免通道哨兵：「」按纪律只装短术语，不进引文核验——这意味着把编造的
# "判决原话"包进「」即可绕过核验层。哨兵不阻断（即使 --strict-quotes），只把
# 超长或带裁判语体特征的「」片段置于可见状态，供人工抽核；命中语料者自动豁免。
TERM_RE = re.compile(r'\u300c([^\u300c\u300d]{2,120})\u300d')
_JUDICIAL_STRONG = ("本院认为", "本院经审理", "经审理查明", "判决如下", "裁定如下", "本院不予")
_JUDICIAL_WEAK = ("应当", "构成", "予以", "认定", "驳回", "承担", "判令")


def check_term_marks(md: str, corpus_norm: str) -> list:
    """返回需人工抽核的「」片段：≥20 字，或含强裁判语体标志，或 ≥12 字且含弱标志。
    不核验 ≠ 不可见——豁免通道的内容必须能被看见。"""
    flagged, seen = [], set()
    for m in TERM_RE.finditer(md):
        t = m.group(1)
        tn = _norm_text(t)
        if tn in seen or not _has_cjk(tn):
            continue
        seen.add(tn)
        suspicious = (len(tn) >= 20
                      or any(k in tn for k in _JUDICIAL_STRONG)
                      or (len(tn) >= 12 and any(k in tn for k in _JUDICIAL_WEAK)))
        if suspicious and tn not in corpus_norm:
            flagged.append(t)
    return flagged
    ...
    for rp in reports:
        ...
        if quotes:
            ...
            if misses:
                ...
                if strict_quotes:
                    any_fail = True
        # 「」哨兵：豁免通道可见化（仅 WARN，不影响退出码）
        flagged = check_term_marks(rp.read_text(encoding="utf-8"), corpus) if corpus else []
        if flagged:
            print(f"   ⚠️  「」哨兵：{len(flagged)} 条超长/裁判语体片段未经核验"
                  f"（「」按纪律只装短术语），请人工抽核是否应为“”逐字引用：")
            for t in flagged[:8]:
