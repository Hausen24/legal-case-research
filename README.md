...
| 形态 | 去重逻辑 | 深度管道 |
|---|---|---|
| **散案**（默认） | 按 `Gid` 去重 | 通用管道（动态争点体系 + 六维统计） |
| **集团/群体性案件** | **按"被告主体＋共同事件"分组**，组内留核心实质判决，平行判决仅留痕 | 长表管道（争点分叉·抗辩↔裁判对应三 sheet Excel）；内置**证券虚假陈述**示范样例 |

四种组合均可（学理×散案 / 学理×集团案 / 实案×散案 / 实案×集团案）。实案模式顺位①由内置的**最高法指导性案例本地数据集**（[data/guiding_cases/](data/guiding_cases/)，官网合规抓取 263 案全文，定期增量更新）提供全文；直辖市多中院映射（朝阳区→北京三中院）由 `court_hierarchy` 解析并在检查点 1 确认；相似性按**三要素加权评分量表**（100 分制）量化留痕。
